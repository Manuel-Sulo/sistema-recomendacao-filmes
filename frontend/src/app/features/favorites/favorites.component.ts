  ...
  template: `
    <div class="page container">
      <div class="page-header animate-fade-up">
        ...
        <h1 class="text-h1">{{ 'favorites.myFavorites' | translate }}</h1>
        <p class="text-muted mt-2">{{ favorites.length }} {{ 'favorites.moviesSaved' | translate }}</p>
      </div>
      <div class="movie-grid mt-8" *ngIf="favorites.length">
        <div *ngFor="let f of favorites" class="fav-item animate-fade-up">
          ...
          <button class="remove-btn btn-icon" (click)="remove(f)" [title]="'actions.remove' | translate">✕</button>
        </div>
      </div>
      <div *ngIf="!favorites.length && !loading" class="empty-state text-center mt-16 animate-fade-up">
        <p style="font-size:56px">♥</p>
        <p class="text-h3 mt-4">{{ 'favorites.noFavorites' | translate }}</p>
        <p class="text-muted mt-2">{{ 'favorites.addMovies' | translate }}</p>
      </div>
  ...
  remove(f: any): void {
    ...
    this.api.removeFavorite(id).subscribe({
      next: () => {
        ...
        this.toast.success(this.t.instant('feedback.removedFavorite'));
      },
