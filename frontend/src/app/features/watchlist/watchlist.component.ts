  ...
  template: `
    <div class="page container">
      <div class="page-header animate-fade-up">
        ...
        <h1 class="text-h1">PARA VER</h1>
        <p class="text-muted mt-2">{{ items.length }} filmes na lista</p>
      </div>
      <div class="movie-grid mt-8" *ngIf="items.length">
        <div *ngFor="let w of items" class="watch-item animate-fade-up">
          ...
          <button class="remove-btn btn-icon" (click)="remove(w)" title="Remover">✕</button>
        </div>
      </div>
      <div *ngIf="!items.length && !loading" class="empty-state text-center mt-16 animate-fade-up">
        <p style="font-size:56px">📋</p>
        <p class="text-h3 mt-4">Watchlist vazia</p>
        <p class="text-muted mt-2">Adicione filmes para ver mais tarde</p>
      </div>
...
export class WatchlistComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.api.getWatchlist().subscribe({
      ...
      error: () => { this.loading = false; this.toast.error('Erro ao carregar watchlist'); },
    });
  }
  ...
  remove(w: any): void {
    ...
    this.api.removeFromWatchlist(id).subscribe({
      next: () => {
        ...
        this.toast.success('Removido da watchlist');
      },
